...
from app.crud import agent_logs, tasks, templates, users
...
tpl = templates.create_template(
    db, name="审查测试模板", description="d", config={"version": "1.0"}
)
...
t = tasks.create_task(
    ...
    input_file_hash="h",
    input_file_path="minio://i/a.docx",
    template_id=tpl.id,
)
...
tasks.mark_success(
    db,
    tid,
    output_file_path="minio://o/mod.docx",
...
check(
    ...
    and t.progress == 100
    and float(t.cost_usd) == 0.05
    and t.completed_at is not None,
)

# 9) 日志
agent_logs.add_log(
    db, task_id=tid, agent_node="executor", message="执行完成", level=LogLevel.INFO
)
agent_logs.add_log(
    db,
    task_id=tid,
...
check(
    "update_task 不存在->None",
    tasks.update_task(db, str(uuid.uuid4()), status=TaskStatus.FAILED) is None,
)
...
tasks.create_task(
    ...
    task_id=tid2,
    prompt_text="p2",
    input_file_name="b.docx",
    input_file_hash="h2",
    input_file_path="minio://i/b.docx",
...
tasks.create_task(
    ...
    task_id=tid3,
    prompt_text="p3",
    input_file_name="c.docx",
    input_file_hash="h3",
    input_file_path="minio://i/c.docx",
    expires_at=datetime.now() - timedelta(hours=1),
)
expired = [x.id for x in tasks.list_expired_tasks(db)]
check(
    "过期清理只命中未终态任务",
    tid3 in expired and tid not in expired and tid2 not in expired,
)
