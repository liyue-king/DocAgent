...
def make_test_docx(file_path: str) -> None:
    ...
    # 正文段落
    p1 = doc.add_paragraph("这是正文段落，用于测试样式修改功能。")
    ...
    p2 = doc.add_paragraph("技术方案包括前端 Vue3 和后端 FastAPI。")
    ...
    p3 = doc.add_paragraph("系统采用五层架构设计。")

    # 还有一个正文段
    p4 = doc.add_paragraph("总结：本文档用于工具层验证测试。")
...
def test_parser(file_path: str) -> dict:
    ...
    styles = set(p["style"] for p in dom["paragraphs"])
...
def test_editor(doc: Document, config: dict) -> None:
    ...
    dom, ops = apply_template(doc, config)
...
def main() -> None:
    ...
    # 生成测试 docx
    file_path = str(Path(__file__).resolve().parent.parent / "data" / "_test_tools.docx")
    Path(file_path).parent.mkdir(exist_ok=True)
    make_test_docx(file_path)

    # 解析
    dom = test_parser(file_path)
