...
class Task(Base):
    ...
    id: Mapped[str] = mapped_column(
        String(36), primary_key=True, comment="UUID (与Celery task_id一致)"
    )
    ...
    input_file_name: Mapped[str] = mapped_column(
        String(255), nullable=False
    )  # 上传文件名
    input_file_hash: Mapped[str] = mapped_column(
    ...
    progress: Mapped[int] = mapped_column(
        TINYINT(unsigned=True), default=0, server_default="0"
    )
    ...
    agent_state_snapshot: Mapped[dict[str, Any] | None] = mapped_column(
        JSON, nullable=True, comment="LangGraph全量状态快照"
    )
